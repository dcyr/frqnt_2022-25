...
tMean_fetch <- function(landtypes, landtypes_AT,
    ...
    if(scenario != "baseline") {
      ...
      #### ## the fetch local .csv obtained from Ouranos (old method)
      ## tMean_project <- read.csv(paste0("../climate/Tmean_", a, ".csv"))
      
      # fetching and formatting simulation data (new format, still CMIP5, from Ouranos)
      dirName <- "../climate/QC_Moyenne-annuelle-des-temperatures_spatial"
      x <- list.files(dirName)
      x <- x[grep("p50", x)]
      if(scenario == "RCP45") {
        s <- 45
      }
      ...
      #### indexing raster files based on years covered
      ### and splicing time series
      x <- x[grep(s, x)]
      x <- x[order(x)]
      years <- as.character(lapply(strsplit(x, "_"), function(x) x[6]))
      yStart <- as.numeric(lapply(strsplit(years, "-"), function(x) x[1]))
      yEnd <- as.numeric(lapply(strsplit(years, "-"), function(x) x[2]))
      years <- data.frame(yStart = as.numeric(lapply(strsplit(years, "-"), function(x) x[1])),
                          yEnd = as.numeric(lapply(strsplit(years, "-"), function(x) x[2])))
      years[, "yMid"] <- apply(years, 1, mean)
      
      
      ### indexing
      breaks <- seq(t0, to = 2100)
      j <- numeric()
     
      for (i in seq_along(breaks)) {
        b <- breaks[i]
        j <- append(j, which.min(abs(years$yMid - b)))
      }
      
      ### going through raster files 
      breaks <- cbind(breaks, j)
      breaks <- breaks[c(1, which(diff(j)>0)+1),]
      r <- stack(paste(dirName, x[breaks[,"j"]], sep = "/"))
      r <- projectRaster(r, landtypes, method = 'ngb')
      r <- resample(r, landtypes)
      tMean_Val <- zonal(r, landtypes, fun = "mean")
      for (i in 1:nrow(breaks)) {
        tmp <- tMean_Val[,c(1,i+1)]
        tmp <- data.frame(year = breaks[i,"breaks"]-t0,
                          zone = tmp[,1],
                          mean = tmp[,2])
        if (!breaks[i,"breaks"] == t0) {
          tMeanOutput <- rbind(tMeanOutput, tmp)  
        }
      }
