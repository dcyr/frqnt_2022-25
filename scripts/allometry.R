...
dhpToAge_fnc <- function(sp,
    ...
    ### extending raster
    rExt <- raster(extend(extent(r), c(100000, 100000,
                                       400000, 50000)),
    ...
    for (g in unique(sppGroup)) {
        ...
        if(nrow(df)<30) {
            ...
            #fit <- lm(dhpSq ~ 0 + age) ## the ones we've been using
            #fit <- lm(dhpSq ~ age)
            #fit <- lm(dhp ~ 0+age)
            fit <- lm(dhp ~ 0 + age)
            modelType <- "linear"
            plot(x = age, y = df$DHP,
                 xlim = c(0, 200), ylim = c(0, max(df$DHP)),
                 main = g)
            
            lines(x = 1:200, y = predict(fit, data.frame(age = (1:200))), type = "l", col = "blue")
            #lines(x = 1:200, y = predict(fit, data.frame(age = (1:200)))^.5, type = "l", col = "blue")
        } else {
            print(paste("fitting Chapman-Richard model for", g, "species group"))
            fit <- sar_chapman(data.frame(a = df$AGE, s = df$DHP),
                               grid_n = 1000,
                               grid_start = "partial")
            modelType <- "Chapman-Richard"
            crPredict <- sar_pred(fit, area = c(1:200))
            plot(x = age, y = df$DHP,
                 xlim = c(0, 200), ylim = c(0, max(df$DHP)),
                 main = g)
            lines(x = crPredict$Area, y = crPredict$Prediction,
                  type = "l", col = "blue")
        }
